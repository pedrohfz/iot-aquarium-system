// ====== Wokwi ESP32 - Tanque A (Temperatura + pH) via MQTT (HiveMQ) ======
#include <WiFi.h>
...
// Tópicos do Tanque A
const char* TOPIC_TELEM  = "aquario/tanqueA/telemetry";
const char* TOPIC_STATUS = "aquario/tanqueA/status";
...
float readTempC() {
  ...
  return 20.0f + (raw / 4095.0f) * 10.0f;
}
float readPH() {
  int raw = analogRead(PH_PIN);
  return (raw / 4095.0f) * 14.0f;
}
...
void connectMQTT() {
  ...
  while (!mqtt.connected()) {
    String clientId = String("esp32-wokwi-tanqueA-") + String(random(0xffff), HEX);
